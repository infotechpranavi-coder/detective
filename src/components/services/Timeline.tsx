  ...
  return (
    <section ref={containerRef} className="relative py-32 overflow-hidden border-t border-foreground/10">
      <CustomImage 
        ...
        overlay="bg-white/70 dark:bg-black/88"
      />
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        >
          ...
          <motion.h2 variants={fadeUp} className="font-playfair text-4xl md:text-5xl lg:text-6xl text-foreground uppercase tracking-wider">
            How We <span className="text-foreground/40 italic">Operate</span>
          </motion.h2>
        </motion.div>

        <div className="relative max-w-4xl mx-auto">
          {/* Vertical Line Desktop */}
          <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-[1px] bg-foreground/10 -translate-x-1/2" />
              ...
              return (
                <motion.div key={i} variants={fadeUp} className={`flex flex-col md:flex-row items-center gap-8 ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                  ...
                  <div className={`w-full md:w-1/2 flex ${isEven ? 'md:justify-end text-left md:text-right' : 'md:justify-start text-left'}`}>
                    <div className="bg-white/82 dark:bg-[#0A0A0A]/80 backdrop-blur-md border border-foreground/10 p-8 max-w-md hover:border-accent/30 transition-colors group shadow-[0_18px_60px_rgba(242,10,10,0.08)]">
                      <div className={`font-bebas text-4xl text-foreground/10 dark:text-white/5 absolute top-4 ${isEven ? 'md:left-4 right-4' : 'right-4 md:right-4'} group-hover:-translate-y-2 transition-transform duration-500`}>{step.num}</div>
                      <h4 className="font-playfair text-xl text-foreground mb-3 uppercase tracking-wider">{step.title}</h4>
                      <p className="font-inter text-sm text-foreground/60 leading-relaxed">{step.desc}</p>
                    </div>
                  </div>

                  {/* Center Node */}
                  <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center justify-center">
                    <div className="w-12 h-12 rounded-full bg-background border-2 border-accent flex items-center justify-center z-10">
